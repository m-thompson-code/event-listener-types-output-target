...
describe("generateStencilEventTypeImport", () => {
    it("should generate import a single component's custom event type", () => {
        expect(
            generateStencilEventTypeImport([stubComponentCompilerMeta({ events: [stubComponentCompilerEvent()] })], "./components"),
        ).toBe(["import {", `${TABS[1]}StubCmpCustomEvent,`, `${TABS[1]}UserImplementedEventType,`, "} from './components';"].join("\n"));
    });
            ...
            generateStencilEventTypeImport(
                [
                    stubComponentCompilerMeta({
                        tagName: "first-cmp",
                        events: [
                            stubComponentCompilerEvent({ complexType: duplicateComplexType }),
                        ],
                    }),
                    stubComponentCompilerMeta({ tagName: "second-cmp", events: [
                        stubComponentCompilerEvent({ complexType: duplicateComplexType }),
                    ], }),
                ],
                "./components",
