...
describe('generateEventMaps', () => {
  it('should generate a event map for a single component', () => {
    expect(generateEventMaps([stubComponentCompilerMeta({ events: [stubComponentCompilerEvent()] })])).toBe(
      ['interface HTMLStubCmpElementEventMap {', `${TABS[1]}stubEvent: StubEvent`, '}'].join('\n'),
    );
  });
