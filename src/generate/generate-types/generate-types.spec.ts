    ...
    return {
        generateStencilEventTypeImport: (cmps: ComponentCompilerMeta[], importPath: string) => {
            return mockGenerateStencilEventTypeImport.mockImplementation(
                (cmps: ComponentCompilerMeta[], importPath: string) =>
                    `/** import stub ${cmps.map((cmp) => cmp.tagName)} from ${importPath} */`,
            )(cmps, importPath);
        },
    };
});
...
jest.mock("../generate-event-maps", () => {
    return {
        generateEventMaps: (cmps: ComponentCompilerMeta[]) => {
            return mockGenerateEventMaps.mockImplementation((cmps: ComponentCompilerMeta[]) =>
            `interface MockGeneratedEventMaps {/** mock ${cmps.map((cmp) => cmp.tagName)} */}`,
            )(cmps);
        },
    };
});
...
jest.mock("../generate-global-type", () => {
    return {
        generateGlobalType: (cmps: ComponentCompilerMeta[]) => {
            return mockGenerateGlobalType.mockImplementation((cmps: ComponentCompilerMeta[]) =>
            `declare global {/** stub ${cmps.map((cmp) => cmp.tagName)} */}`,
            )(cmps);
        },
    };
});
...
describe("generateTypes", () => {
    ...
    it("should return just comments if no components", () => {
        expect(generateTypes([], 'some-import-path')).toBe([
            '/** stub comments */',
            '',
        ].join('\n'));
    });
